...
# Конфигурация
BOT_TOKEN = "8538861121:AAHlTZ32yokj2L-D93VRREXbs5ZxFf3wdro"
...
async def buy_review_handler(callback: CallbackQuery, state: FSMContext):
    user = db.get_user(callback.from_user.id)
    ...
    await state.set_state(Form.waiting_for_review_reward)
...
async def process_reward(message: types.Message, state: FSMContext):
    ...
    await state.set_state(Form.waiting_for_review_count)
    await message.answer(
...
async def process_keywords(message: types.Message, state: FSMContext):
    ...
    await state.set_state(Form.waiting_for_banned_words)
    await message.answer(
...
async def process_banned_words(message: types.Message, state: FSMContext):
    ...
    await state.set_state(Form.waiting_for_group_id)
    await message.answer(
        "👥 <b>ID группы</b>\n\n"
...
async def process_group_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"❌ Ошибка проверки группы: {str(e)[:100]}\n"
...
async def process_amount(message: types.Message, state: FSMContext):
    try:
        ...
        if amount < 10 or amount > 1000:
            await message.answer(
                "❌ Сумма должна быть от 10 до 1000₽!\n"
                "Введите сумму заново:",
                parse_mode=ParseMode.HTML
            )
            return
...
async def process_withdraw_details(message: types.Message, state: FSMContext):
    ...
    admin_message = await bot.send_message(
        ...
        text=f"📤 <b>Новая заявка на вывод</b>\n\n"
             ...
             f"💳 Способ: {method_names[method]}\n"
             f"📋 Реквизиты: {details}\n"
             f"💸 Баланс пользователя: {user['balance']:.2f}₽",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
...
async def admin_change_balance_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(Form.admin_change_balance)
...
async def process_admin_change_balance(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Обновляем баланс
        updated_user = db.update_balance(user_id, amount)
        ...
        await message.answer(
            ...
            f"💳 Новый баланс: {updated_user['balance']:.2f}₽",

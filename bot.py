...
# Вспомогательная функция для отправки сообщений с HTML разметкой
async def send_message(chat_id: int, text: str, **kwargs):
    if 'parse_mode' not in kwargs:
        kwargs['parse_mode'] = ParseMode.HTML
    return await bot.send_message(chat_id, text, **kwargs)

async def edit_message_text(chat_id: int, message_id: int, text: str, **kwargs):
    if 'parse_mode' not in kwargs:
        kwargs['parse_mode'] = ParseMode.HTML
...
async def cmd_admin(message: types.Message):
    if message.from_user.id == ADMIN_CHAT_ID:
        await send_message(message.chat.id, "Админ панель:", reply_markup=admin_menu())
    else:
        await send_message(message.chat.id, "У вас нет доступа к админ панели")
...
async def process_review_text(message: types.Message, state: FSMContext):
    ...
    if not order_info:
        await send_message(message.chat.id, "❌ Заказ не найден!")
        await state.clear()
        return
...
async def process_withdraw_details(message: types.Message, state: FSMContext):
    ...
    # Отправляем админу
    user = db.get_user(message.from_user.id)
    admin_message = await send_message(
        chat_id=ADMIN_CHAT_ID,
...
async def process_admin_mailing(message: types.Message, state: FSMContext):
    ...
    for user in users:
        try:
            await send_message(user[0], message.text)
            success += 1
        except:
            failed += 1
